...
def jwk_set_router(usecase: JWKSetUseCase, algorithm: str) -> APIRouter:
    ...
    def jwk_set() -> JWKSetResponse:
        keys = [
            JWKKeyResponse.from_key_pair(key_pair, algorithm)
            for key_pair in usecase.get_active_keys()
        ]
