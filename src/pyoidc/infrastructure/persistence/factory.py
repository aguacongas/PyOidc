...
stores (clés, clients, codes) : il est dérivé de ``key_store_type`` /
``key_store_dsn``. Chaque fabrique retourne le port correspondant, ce qui
permet à la composition root d'injecter des implémentations différentes
sans toucher aux usecases.
"""

from __future__ import annotations

from pyoidc.infrastructure.settings import Settings
from pyoidc.interfaces.repositories.authorization_code_repository import (
    AuthorizationCodeRepository,
)
from pyoidc.interfaces.repositories.client_repository import ClientRepository
...
def build_client_repository(settings: Settings) -> ClientRepository:
    ...
    if settings.key_store_type == "memory":
        from pyoidc.infrastructure.persistence.clients_memory import (
            InMemoryClientRepository,
        )

        return InMemoryClientRepository()
    if settings.key_store_type == "sql":
        from pyoidc.infrastructure.persistence.clients_sql import SQLClientRepository
...
def build_authorization_code_repository(settings: Settings) -> AuthorizationCodeRepository:
    ...
    if settings.key_store_type == "memory":
        from pyoidc.infrastructure.persistence.codes_memory import (
            InMemoryAuthorizationCodeRepository,
        )

        return InMemoryAuthorizationCodeRepository()
    if settings.key_store_type == "sql":
        from pyoidc.infrastructure.persistence.codes_sql import (
